//! SHA-256 for Mach-O page hashes (FIPS 180-4). Kept in-tree so signing does
//! not require a crypto crate, OS signing service, or a codesign subprocess.
pub fn digest(input: &[u8]) -> [u8; 32] {
    ...
    for (bytes, value) in result.chunks_exact_mut(4).zip(state) {
