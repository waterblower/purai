...
训练数据由 `.gitignore` 排除。Exp4 的 WikiText-103 raw 下载和预处理：

```sh
sh scripts/download_wikitext_100mb.sh
```

该脚本把约 100 MB 完整文章和官方验证集写入 `../data/`，使用 Rust 转换官方 Parquet，无 Python 依赖。模型、日志和报告放在 [../exp4](../exp4/README.md)，其中包含完整训练和评测复现步骤。
...
- 默认验证集只报告指标，不选择候选结构、训练轮数或最佳 checkpoint，也不更新校准表。
- 可选 `--stop-on-bpb-increase`：从第 2 轮起，当前轮验证集 `graph_only` BPB 首次严格大于上一轮时立即停止，保存上一轮模型。比较完整浮点精度，相等继续；触发停止的轮次也保留在日志和报告中。此时验证集用于选择训练轮数，属于开发集，不再是独立测试集。`--rounds` 仍是最大轮数，默认不启用此停止条件。
