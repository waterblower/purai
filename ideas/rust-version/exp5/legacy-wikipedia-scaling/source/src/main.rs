...
train:
  ...
  --stop-on-bpb-increase  stop at first strict rise in validation graph BPB; save previous round
infer:
...
#[derive(Default)]
struct BpbEarlyStop {
    previous: Option<(usize, f64, Model)>,
}

impl BpbEarlyStop {
    fn observe(&mut self, model: &Model, round: usize, bpb: f64) -> bool {
        if self
            .previous
            .as_ref()
            .is_some_and(|(_, previous, _)| bpb > *previous)
        {
            return true;
        }
        self.previous = Some((round, bpb, model.clone()));
        false
    }
}
...
fn train_command(words: &[String]) -> Result<()> {
    ...
    let use_early_stop = a.flag("stop-on-bpb-increase");
    let mut early_stop = BpbEarlyStop::default();
    let mut stopped_on_increase = false;
    let mut model = train::train_controlled(&training, params.clone(), &options, |m, r| {
        ...
        if use_early_stop && early_stop.observe(m, r.round, eval.bpb()) {
            stopped_on_increase = true;
            let (previous_round, previous_bpb, _) = early_stop.previous.as_ref().unwrap();
            println!(
                "early stop: round {} validation graph BPB {:.12} > round {} {:.12}; restoring round {}",
                r.round,
                eval.bpb(),
                previous_round,
                previous_bpb,
    ...
    let completed_rounds = rounds.len();
    let mut selected_round = completed_rounds;
    if stopped_on_increase {
        let (round, _, checkpoint) = early_stop.previous.take().unwrap();
        model = checkpoint;
        selected_round = round;
    }
    // Release the spare checkpoint before compilation and the n-gram baseline.
    drop(early_stop);
    let stop_reason = if stopped_on_increase {
        "validation_bpb_increased"
    } else if rounds
        .last()
        .is_some_and(|r| r["added"] == 0 && r["merged"] == 0 && r["pruned"] == 0)
    {
    ...
    "stopping":{"stop_on_bpb_increase":use_early_stop,"reason":stop_reason,"completed_rounds":completed_rounds,
        "selected_round":selected_round,"comparison":"strict current > previous, full f64 precision, graph-only validation BPB; equality continues; first round establishes the reference",
        "selected_valid_graph_bpb":rounds.get(selected_round.wrapping_sub(1)).map(|r|r["valid_graph_bpb"].clone())},
    ...
    "protocol":if use_early_stop {
        "Document boundaries reset all history. Calibration is withheld from training input. Validation graph BPB selects stopping and the previous-round checkpoint at its first strict increase; it does not choose individual structural edits or copy calibration. Validation is now a development set, not an independent final test. Metrics are byte-weighted."
    } else {
        "Document boundaries reset all history. Calibration is withheld from the training input. Validation is reporting-only: it does not choose edits, epochs, parameters or calibration. Metrics are byte-weighted. No validation-based best-checkpoint selection."
    }});
...
mod tests {
    ...
    #[test]
    fn early_stop_keeps_previous_checkpoint_at_first_strict_increase() {
        let mut stop = BpbEarlyStop::default();
        let first = Model::root(Params::default(), &[b"abc".to_vec()]);
        let second = Model::root(Params::default(), &[b"different corpus".to_vec()]);
        assert!(!stop.observe(&first, 1, 2.0));
        assert!(!stop.observe(&first, 2, 1.5));
        assert!(!stop.observe(&second, 3, 1.5));
        // Even an increase hidden by the table's five decimal places must stop.
        assert!(stop.observe(&first, 4, f64::from_bits(1.5f64.to_bits() + 1)));
        let (round, bpb, checkpoint) = stop.previous.unwrap();
        assert_eq!(round, 3);
        assert_eq!(bpb, 1.5);
        assert_eq!(checkpoint.training_bytes, second.training_bytes);
        assert_ne!(checkpoint.training_bytes, first.training_bytes);
    }
}
