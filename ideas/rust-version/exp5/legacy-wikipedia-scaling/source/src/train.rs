...
pub fn train(
    ...
    train_controlled(docs, params, opts, |model, report| {
        on_round(model, report);
        ControlFlow::Continue(())
    })
}

/// Stop immediately after a completed round when the callback requests it.
/// The caller owns checkpoint selection; the returned model is the last evaluated round.
pub fn train_controlled(
    docs: &[Vec<u8>],
    params: Params,
    opts: &Options,
    mut on_round: impl FnMut(&Model, &RoundReport) -> ControlFlow<()>,
) -> Result<Model> {
    params.validate()?;
    opts.validate()?;
    ...
    for round in 1..=opts.rounds {
        ...
        if on_round(&model, &report).is_break() {
            break;
        }
